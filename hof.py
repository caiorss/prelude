...
class Functor(object):
    ...
    def __call__(self, *args):
        ...
        if len(argvalues) == self.nargs:
            ...
            newfunctor = Functor(self.funct, argvalues)
            return newfunctor            
...
def zipwith(func, *Lists):
    ...
    """
    return [func(*row) for row in zip(*Lists)]
...
class Pipe(object):
    """
    This class implements the F#( F Sharp) pipe operator.
    
    
    >>> Pipe() | 10.23 | (lambda x: x**2) | (lambda y: y/10) | None
        10.46529

    >>> Pipe() >> [1, 2, 4, 5]  >> (lambda x: x**2) >> (lambda y: y/10) >> list
    [0.10000, 0.40000, 1.60000, 2.50000]
    
    Pipe() >> [1, 2, 4, 5]  >> (lambda x: x**2) >> (lambda y: y/10) | sum | list
    4.60000
